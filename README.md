...
4. In Vercel, set every variable above (use a long random `CRON_SECRET`) and deploy. `vercel.json` runs functions in `hnd1` (Tokyo), next to the Supabase project in `ap-northeast-1`; change `regions` if your Supabase project is elsewhere. `vercel.json` schedules `/api/cron/daily` at 13:00 UTC. On the Hobby plan it runs once at some point within that hour. A run where any firm or email fails returns an error status, so it shows as failed in the cron logs.
