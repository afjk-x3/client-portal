...
- Vitest: 53 unit tests pass, and the daily job's 3 integration tests pass against the local stack. `tsc`, ESLint, and `next build` with Cache Components all pass.
...
11. **Resend batches** are sent in permissive mode, so one rejected address never drops the rest, spaced 600 ms apart across the process to stay under Resend's default 2 requests per second, and retried once when Resend reports the rate limit anyway. Every email is built before the state change or cron claim it belongs to, so a configuration error changes nothing.
