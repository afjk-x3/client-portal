...
|---|---|---|---|
| 1 | [Foundation](2026-09-24-client-portal-1-foundation.md) | Next.js scaffold, shadcn/ui, pure libraries (dates, reminders, redirect paths, files, validation, errors, email) | 31 Vitest tests pass; build passes |
...
- Vitest: 31 tests pass. `tsc`, ESLint, and `next build` with Cache Components all pass.
...
4. **No react-hook-form.** shadcn's `Form` component is built on react-hook-form, which conflicts with `useActionState` and would be a new runtime dependency. Forms use plain inputs with `maxLength` and `required` taken from the same limits as the zod schemas, and the Server Action validates with zod.
...
11. **Resend batches** are spaced 600 ms apart to stay under Resend's default 2 requests per second.
