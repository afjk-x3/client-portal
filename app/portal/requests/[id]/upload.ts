...
const RETRY = "Upload failed. Check your connection and retry.";
 ...
 * Never throws, so callers' upload queues keep moving.
...
export async function uploadFile(itemId: string, file: File): Promise<string | null> {
  try {
    ...
    // A dropped connection or a new deployment makes an action call throw.
    return RETRY;
  }
}
